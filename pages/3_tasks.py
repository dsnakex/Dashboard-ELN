...
Complete task management with CRUD and comments.
"""
...
def show_create_task_form(subprojects, users):
    ...
    with st.form("create_task_form", clear_on_submit=True):
        ...
        with col1:
            title = st.text_input("Task title *", placeholder="Ex: Analyze in vitro test results")
        ...
        with col2:
            ...
            estimated_hours = st.number_input("Estimated hours", min_value=0.0, step=0.5, value=0.0)
...
def show_task_card(task, users):
    ...
    with st.container():
        ...
        with col2:
            ...
            st.markdown(f"**Priority:** {priority_icons.get(task['priority'], '⚪')} {task['priority']}")
...
def show_comments_section(task_id):
    ...
    if has_permission('create'):
        with st.form(f"add_comment_{task_id}", clear_on_submit=True):
            content = st.text_area("Add a comment", placeholder="Your comment...")
...
def main():
    ...
    st.markdown("Create and manage your daily tasks")
    st.markdown("---")
    ...
    if not subprojects:
        st.warning("⚠️ No subprojects available. Create a project and subproject first.")
        return
    ...
    if view_mode == 'my_tasks':
        ...
        tasks = get_all_tasks()
        st.markdown(f"### 📋 All Tasks ({len(tasks)})")
    ...
    with col3:
        search = st.text_input("🔍 Search", placeholder="Task title...")
