-- ============================================
-- NIKAIA ELN - SETUP FINAL AVEC NETTOYAGE COMPLET
-- ============================================
-- Ce script supprime TOUTES les policies (y compris storage)
-- puis crée uniquement les policies des tables publiques
-- ============================================
...
-- ============================================
-- ÉTAPE 1: SUPPRESSION AGRESSIVE DE TOUTES LES POLICIES
-- ============================================

-- Supprimer TOUTES les policies de storage.objects (même celles avec entity_type)
DO $$
DECLARE
    r RECORD;
BEGIN
    -- Supprimer policies storage
    FOR r IN (
        SELECT policyname
        FROM pg_policies
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not drop storage policy: % - %', r.policyname, SQLERRM;
        END;
    END LOOP;

    -- Supprimer policies publiques
    FOR r IN (
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not drop public policy: % - %', r.policyname, SQLERRM;
        END;
    END LOOP;
END $$;

-- ============================================
-- ÉTAPE 2: SUPPRESSION DES CONTRAINTES FK
-- ============================================
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_experiments_protocol') THEN
    ALTER TABLE experiments ADD CONSTRAINT fk_experiments_protocol
    FOREIGN KEY (protocol_id) REFERENCES protocols(id) ON DELETE SET NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_experiments_template') THEN
    ALTER TABLE experiments ADD CONSTRAINT fk_experiments_template
...
CREATE TRIGGER update_experiment_templates_updated_at BEFORE UPDATE ON experiment_templates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
SELECT
  ...
  'Note: Aucune policy storage créée. Configurez le bucket avatars manuellement si nécessaire.' AS note,
